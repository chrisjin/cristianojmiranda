...
privileged aspect Usuario_Roo_Entity {
	...
	public void Usuario.remove() {
		...
		if (this.entityManager.contains(this)) {
			...
			Usuario attached = this.entityManager
	...
	public static long Usuario.countUsuarios() {
		return ((Number) entityManager().createQuery(
				"select count(o) from Usuario o").getSingleResult())
				.longValue();
	}

	@SuppressWarnings("unchecked")
	public static List<Usuario> Usuario.findAllUsuarios() {
		return entityManager().createQuery("select o from Usuario o")
				.getResultList();
	}

	@SuppressWarnings("unchecked")
	public static List<Usuario> Usuario.findUsuarioByLoginAndPass(String login,
			String pass) {
		...
		try {
			...
			Query query = entityManager()
					.createQuery(
							"select o from Usuario o where trim(upper(o.dsLogin)) = trim(upper(:login)) and trim(upper(dsSenha)) = trim(upper(:pass))");

			// Bind dos parametros da consulta
	...
	@SuppressWarnings("unchecked")
	public static List<Usuario> Usuario.findUsuarioEntries(int firstResult,
			int maxResults) {
		return entityManager().createQuery("select o from Usuario o")
