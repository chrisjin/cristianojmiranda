 ...
 *     All Rights Reserved.
 *
 *  Created on : Oct 01, 2011 by cmiranda
 *
...
RA083382::ResultType RA083382::solve(const double max_time) {
	cout
			...
			<< max_time << " segundos." << endl;

	algExato.exato1(&graph, &terminal, &length);
...
RA083382::ResultType RA083382::solveFast(const double max_time) {
	cout << "\n--> Resolvendo mega rapido e magico algoritmo em " << max_time
			<< " segundos." << endl;
...
void RA083382::chutaCiclo() {
	...
	for (ListGraph::EdgeIt e(graph); e != INVALID; ++e)
		cout << "(" << graph.id(graph.u(e)) << "," << graph.id(graph.v(e))
				<< "): " << length[e] << " | ";
