...
int main() {
    ...
    while(1) {
		...
		// Processo principal não referencia a nova conexao
        close(new_fd);
...
void ler_comando(int new_fd) {
	...
	// Obtem o usuario
	usuario = obterUsuarioPorDocumento(atoi(comando[0]));
	if (usuario == NULL){
	...
	if (strcmp(comando[1], OBTER_TODOS_ISBNS) == 0) {
		...
		char* descricao = obterDescricaoPorISBN();
